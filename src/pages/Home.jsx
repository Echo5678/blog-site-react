...
export default function Home() {
  ...
  const handleInputSearch = (e) => {
    ...
    axios.get(`http://localhost:1337/api/posts?filters[title][$contains]=${searchValue}&sort=date:${sortType}&pagination[page]=0&pagination[pageSize]=${POST_LIMIT}`).then(function (response) {
      const responseData = response.data.data;
      setPosts(responseData);
    }).catch(function(error) {
      setPosts("Error receiving data");
  ...
  <div className="mx-auto max-w-3xl w-11/12 md:w-2/3">
    <PostSearchBar handleFormSearch={handleFormSearch} handleInputSearch={handleInputSearch} toggleSort={toggleSort} sortType={sortType} search={search}  />
    { 
      posts.length > 0 ?
      posts.map((post, key) => {
